...
def test_research_lead_node_passes_existing_research_and_repair_context(
    ...
    monkeypatch.setattr(
        ...
        lambda execution_brief, existing_research=None, repair_context=None: (
            captured.update(
                {
...
def test_draft_writer_node_records_review_repair_trigger(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda execution_brief, claim_ledger, analytical_backbone, content_blueprint, repair_context=None: (
            "draft"
        ),
    )
